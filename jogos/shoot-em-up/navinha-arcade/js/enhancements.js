...
spawnPowerup = function(x,y) {
  ...
  if(roll<.18){ const types=['overdrive','drone','bomb']; const type=types[Math.floor(Math.random()*types.length)]; powerups.push({x,y,w:22,h:22,type,speed:2,life:420}); return; }
  baseSpawnPowerup(x,y);
};
const baseDrawPowerups = drawPowerups;
drawPowerups = function(){
  baseDrawPowerups();
  powerups.forEach(p=>{
    if(!['overdrive','drone','bomb'].includes(p.type)) return;
    ctx.save();ctx.translate(p.x+11,p.y+11);ctx.shadowBlur=12;
    if(p.type==='overdrive'){ctx.strokeStyle='#ff8a00';ctx.shadowColor='#ff8a00';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-7,7);ctx.lineTo(0,-9);ctx.lineTo(2,-1);ctx.lineTo(8,-1);ctx.lineTo(0,10);ctx.lineTo(-1,2);ctx.lineTo(-7,2);ctx.closePath();ctx.stroke();}
    if(p.type==='drone'){ctx.strokeStyle='#b366ff';ctx.shadowColor='#b366ff';ctx.lineWidth=2;ctx.strokeRect(-8,-6,16,12);ctx.beginPath();ctx.moveTo(-8,0);ctx.lineTo(-13,5);ctx.moveTo(8,0);ctx.lineTo(13,5);ctx.stroke();}
    if(p.type==='bomb'){ctx.fillStyle='#ff3b3b';ctx.shadowColor='#ff3b3b';ctx.beginPath();ctx.arc(0,2,8,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#fff';ctx.beginPath();ctx.moveTo(3,-6);ctx.quadraticCurveTo(8,-12,10,-7);ctx.stroke();}
    ctx.restore();
  });
};
