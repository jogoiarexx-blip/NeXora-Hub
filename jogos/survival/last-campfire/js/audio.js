...
const sfx = (function(){
  ...
  function syncGameLoops(){
    if(!G || !G.running || G.paused || !save.settings.sound){ stopAllLoops(); return; }
    ensure();
    startLoop('fire', fireLoop); startLoop('wind', windLoop); startLoop('rain', rainLoop);
  }
  function fireLoop(){
  ...
  return { ensure, playClick, playPop, playChime, playVictory, playDefeat, startLoop, stopLoop, stopAllLoops, syncGameLoops, fireLoop, windLoop, rainLoop, setVolume, loops };
})();
