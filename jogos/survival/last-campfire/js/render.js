...
function draw(){
  ...
  drawFog();
  drawLightningFlash();
  if(G.dangerFlash>0){ctx.save();ctx.globalAlpha=G.dangerFlash*.28;ctx.fillStyle='#ff3b30';ctx.fillRect(0,0,W,H);ctx.restore();}
  drawVignette();
}
...
function drawBackgroundForest(){
  ...
  ctx.save();
  const layers=W>760?2:1;
  for(let layer=0;layer<layers;layer++){
    const count=Math.ceil(W/(layer?70:95))+2;
    ctx.globalAlpha=layer?0.22:0.32;
    ctx.fillStyle=layer?'#101b18':'#0b1413';
    for(let i=0;i<count;i++){
      const x=(i* (layer?70:95) + (layer?26:0))% (W+100)-40;
      const h=(layer?75:105) + ((i*37)%55);
      ctx.beginPath();
      ctx.moveTo(x-22,horizon); ctx.lineTo(x,horizon-h); ctx.lineTo(x+22,horizon); ctx.closePath(); ctx.fill();
      ctx.fillRect(x-3,horizon-h*0.55,6,h*0.55);
    }
  }
  ctx.restore();
}
...
function drawWood(){
  for(const w of G.woodPieces){
    ...
    if(G.selectedWoodId===w.id){ ctx.shadowColor='#ffcf7a'; ctx.shadowBlur=22; }
    else if(w.dragging){ ctx.shadowColor='#000a'; ctx.shadowBlur=10; }
    drawLog(w.def.size, w.def.color, w.type==='dourado');
    if(w.type==='molhada'){ctx.rotate(-w.rot);ctx.fillStyle='#9bd8e8';ctx.font='bold 11px sans-serif';ctx.fillText('💧',-6,-w.def.size-5);}
    if(w.type==='resina'){ctx.rotate(-w.rot);ctx.fillStyle='#ffd07a';ctx.font='bold 11px sans-serif';ctx.fillText('✨',-6,-w.def.size-5);}
    ctx.restore();
    if(w.dragging){
      ctx.save();
...
function drawAnimal(){
  if(!G.animal) return;
  const a = G.animal, type=a.type||'raccoon';
  ctx.save(); ctx.translate(a.x,a.y); if(a.phase==='leave' && (type==='wolf'||type==='boar')) ctx.scale(-1,1);
  const sc=a.scale||1; ctx.scale(sc,sc);
  if(type==='raccoon'){
    ctx.fillStyle='#555961'; ctx.beginPath();ctx.ellipse(0,0,15,8,0,0,7);ctx.fill();
    ctx.fillStyle='#777b83';ctx.beginPath();ctx.arc(12,-4,6,0,7);ctx.fill();
    ctx.fillStyle='#202226';ctx.fillRect(8,-8,10,4);ctx.beginPath();ctx.arc(15,-5,1.6,0,7);ctx.fill();
    ctx.strokeStyle='#555961';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-12,0);ctx.quadraticCurveTo(-25,-7,-29,2);ctx.stroke();
  } else if(type==='fox'){
    ctx.fillStyle='#a85a2d';ctx.beginPath();ctx.ellipse(0,0,17,8,0,0,7);ctx.fill();
    ctx.beginPath();ctx.moveTo(10,-5);ctx.lineTo(20,-11);ctx.lineTo(19,1);ctx.closePath();ctx.fill();
    ctx.fillStyle='#f0d0a5';ctx.beginPath();ctx.moveTo(18,-3);ctx.lineTo(24,-1);ctx.lineTo(18,1);ctx.closePath();ctx.fill();
    ctx.strokeStyle='#b96b39';ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(-14,-1);ctx.quadraticCurveTo(-28,-10,-31,3);ctx.stroke();
  } else if(type==='wolf'){
    ctx.fillStyle='#606875';ctx.beginPath();ctx.ellipse(0,-2,19,10,0,0,7);ctx.fill();
    ctx.beginPath();ctx.moveTo(12,-8);ctx.lineTo(23,-13);ctx.lineTo(22,2);ctx.closePath();ctx.fill();
    ctx.fillStyle='#c7d4c9';ctx.beginPath();ctx.arc(20,-6,1.8,0,7);ctx.fill();
    ctx.strokeStyle='#4b525d';ctx.lineWidth=5;for(const lx of [-9,8]){ctx.beginPath();ctx.moveTo(lx,4);ctx.lineTo(lx,12);ctx.stroke();}
  } else {
    ctx.fillStyle='#4b3b32';ctx.beginPath();ctx.ellipse(0,-1,22,12,0,0,7);ctx.fill();
    ctx.beginPath();ctx.arc(19,-3,9,0,7);ctx.fill();
    ctx.strokeStyle='#e5d5b7';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(25,0);ctx.lineTo(31,5);ctx.moveTo(24,-1);ctx.lineTo(30,-6);ctx.stroke();
    ctx.strokeStyle='#3d2f28';ctx.lineWidth=6;for(const lx of [-10,10]){ctx.beginPath();ctx.moveTo(lx,7);ctx.lineTo(lx,15);ctx.stroke();}
  }
  ctx.restore();
}
...
function drawBoss(){
  if(!G.boss) return;
  const b=G.boss;
  ctx.save(); ctx.globalAlpha=Math.max(0,Math.min(1,b.alpha)); ctx.translate(b.x,b.y-18);
  const pulse=1+Math.sin(G.time*8)*0.035; ctx.scale(pulse,pulse);
  ctx.shadowColor='#8fc8ff';ctx.shadowBlur=18;
  ctx.fillStyle='#19202d';ctx.beginPath();ctx.ellipse(0,0,46,25,0,0,7);ctx.fill();
  ctx.beginPath();ctx.moveTo(-5,-17);ctx.lineTo(30,-33);ctx.lineTo(41,-7);ctx.closePath();ctx.fill();
  ctx.fillStyle='#9ad9ff';ctx.shadowBlur=10;ctx.beginPath();ctx.arc(29,-18,3.5,0,7);ctx.fill();
  ctx.strokeStyle='#6eb8ff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-32,-15);ctx.lineTo(-46,-31);ctx.lineTo(-37,-28);ctx.lineTo(-51,-45);ctx.stroke();
  ctx.shadowBlur=0;ctx.strokeStyle='#111722';ctx.lineWidth=10;for(const lx of [-22,10]){ctx.beginPath();ctx.moveTo(lx,16);ctx.lineTo(lx,35);ctx.stroke();}
  ctx.restore();

  // barra do chefe no canvas — funciona igual em PC e mobile
  const bw=Math.min(420,W*0.68), bh=12, bx=(W-bw)/2, by=Math.max(76,H*0.11);
  ctx.save();ctx.fillStyle='rgba(0,0,0,.64)';ctx.fillRect(bx-5,by-22,bw+10,40);
  ctx.fillStyle='#e8eef6';ctx.font='700 12px system-ui';ctx.textAlign='center';ctx.fillText('🐺⚡ BESTA DA TEMPESTADE',W/2,by-7);
  ctx.fillStyle='#2b171b';ctx.fillRect(bx,by,bw,bh);ctx.fillStyle='#b93345';ctx.fillRect(bx,by,bw*Math.max(0,b.hp/b.maxHp),bh);
  ctx.strokeStyle='rgba(255,255,255,.28)';ctx.strokeRect(bx,by,bw,bh);ctx.restore();
}
...
function drawFire(){
  ...
  if(G.dragging || G.selectedWoodId){
    ctx.save();
    const pulse=0.45+0.2*Math.sin(G.time*7);
    ctx.strokeStyle=`rgba(255,207,122,${pulse})`; ctx.lineWidth=3;
    ctx.setLineDash([8,7]);
    ctx.beginPath(); ctx.ellipse(fx,fy-24,68,45,0,0,Math.PI*2); ctx.stroke();
